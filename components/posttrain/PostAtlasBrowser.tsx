...
export default function PostAtlasBrowser() {
  const [domain, setDomain] = useState("foundations");
  const [openId, setOpenId] = useState<string | null>(null);
  const topics = POST_TOPICS.filter((t) => t.domain === domain);

  return (
    <div>
      <div className="atlasTabs" role="tablist" aria-label="Post-training domains">
        {POST_DOMAINS.map((d) => (
      ...
      <p className="sec-sub" style={{ marginTop: "var(--s4)" }}>
        {POST_DOMAINS.find((d) => d.id === domain)?.blurb}
      </p>

      <div className="atlasList">
        {topics.map((t) => {
          const open = openId === t.id;
          const color = DOMAIN_COLORS[t.domain] || "var(--cyan)";
          return (
                ...
                <ul style={{ margin: "12px 0 0", padding: 0, listStyle: "none" }}>
                  ...
                  {t.kind === "scene" && t.scene && (
                    <li style={{ paddingTop: 10 }}>
                      <Link className="linkArrow" href={`/posttrain/scenes/${t.scene}/`}>
                        OPEN THE EXHIBIT →
                      </Link>
                    </li>
                  )}
